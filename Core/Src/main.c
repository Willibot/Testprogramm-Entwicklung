...
// ------------------- Konfiguration -------------------
#define HOLD_THRESHOLD_MS 2000 // Ab wann Chase startet
#define CHASE_MAX_DURATION 2000 // Chase läuft max. 2 Sekunden
...
void handle_touch_events(void) {
        ...
        if (!any_button_pressed && now_pressed) {
            for (int i = 0; i < 4; ++i) { // nur T0..T3
                ...
                if ((BUTTON_MASK & mask) && (status & mask)) {
                    ...
                    // Outputs setzen
                    drv8904q1_set_outputs(out_states[i][0], out_states[i][1]);
    ...
    for (int i = 0; i < 4; ++i) {
        if (button_press_timestamp[i] &&
            ...
            // Chase starten
            led_effect_hold_multibutton_chase_left_start(hue_map[i], 255);
            chase_running = true;
            chase_start_time = HAL_GetTick();
        }
    }

    // Chase beenden
    if (chase_running && (HAL_GetTick() - chase_start_time >= CHASE_MAX_DURATION)) {
        chase_running = false;
